...
if (isset($_POST['selecionarano'])) {
	...
	$selecionarano = $_POST['selecionarano'];
		...
		$sql = mysql_query("UPDATE anoselecionado SET anoselecionado_mes_pagos = '".$selecionarano."'");
		...
		$seleciona = mysql_query("SELECT anoselecionado_mes_pagos FROM anoselecionado LIMIT 1");
...
if($mesatual == 12){$cordez = "class='icon-calendar' style='float:right;font-size:25px;margin:5px 10px 0 0;'";}
			...
			$seleciona = mysql_query("SELECT * FROM total_pago_mes WHERE ano = ".$anoselecionado."");
			while($ln = mysql_fetch_array($seleciona)){
			$ano = $ln['ano'];
			$jan = $ln['jan'];
			$fev = $ln['fev'];
